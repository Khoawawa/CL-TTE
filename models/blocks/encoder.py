...
class ContrastiveEncoder(nn.Module):
    ...
    def detect_potential_overlap(self, link_index):
        """
        Vectorized overlap detector that handles padding tokens safely
        without triggering full 4D memory allocation blowups.
        """
        B_size, T_size = link_index.shape
        ignore_mask = torch.zeros(B_size, B_size, dtype=torch.bool, device=link_index.device)
        
        # Track authentic non-padded trajectory indices
        ...
        for i in range(B_size):
            ...
            row_i = link_index[i].unsqueeze(1)  # (T, 1)
            
            # Broadcast matches across all batch rows (B, T, T)
            matches = (link_index.unsqueeze(1) == row_i.unsqueeze(0))
            
            # Mask out alignments generated by padding pairs
            matches = matches & valid_mask.unsqueeze(1) & (row_i != 0).unsqueeze(0)
            
            # Identify unique links from segment i found in segment j
            intersect_counts = matches.any(dim=2).sum(dim=1).float()
            
            min_lens = torch.minimum(valid_lens[i], valid_lens)
            overlap_ratio = intersect_counts / (min_lens + 1e-6)
            
            ignore_mask[i] = overlap_ratio > 0.8
    ...
    def forward(
        ...
        if self.training:
            ...
            ignore_mask = self.detect_potential_overlap(link_index)
